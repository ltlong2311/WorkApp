...
export default class CollapsibleList extends Component {
    ...
    animate(toValue, callback) {
        ...
        switch (animationType) {
            case 'spring':
                Animated.spring(animation, {...animationConfig, toValue}).start(
                    callback,
                );
                break;
            case 'timing':
                Animated.timing(animation, {...animationConfig, toValue}).start(
                    callback,
                );
                break;
        }
    ...
    onItemLayout(event) {
        ...
        if (!calculationCompleted) {
            this.setState(
                prevState => ({maxHeight: prevState.maxHeight + height}),
                () => {
                    if (calculatedItems < React.Children.count(children)) {
                        this.setState(prevState => ({
                            calculatedItems: prevState.calculatedItems + 1,
                        }));
                    }
    ...
    render() {
        const {animation, initialized} = this.state;
        const {numberOfVisibleItems, buttonContent, wrapperStyle, children} =
            this.props;
